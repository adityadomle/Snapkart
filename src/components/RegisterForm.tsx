...
function RegisterForm({ previousStep }: propType) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false)
      alert("User registered successfully!") // ✅ Optional success alert
    } catch (error: any) {
      if (error.response && error.response.data) {
        alert(error.response.data.message) // ✅ Show backend error nicely
      } else {
        console.log(error)
        setLoading(false)
      }
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative'>
      ...
      <p className='text-gray-600 mb-8 flex items-center'>
        Join SnapKart Today <Leaf className='w-5 h-5 text-green-600' />
      </p>
      ...
      >
        ...
        {
          (() => {
            const formValidation = name !== "" && email !== "" && password !== "" // ✅ include email
